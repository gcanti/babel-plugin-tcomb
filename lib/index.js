...
exports['default'] = function (_ref) {
  ...
  function getList(node) {
    return t.callExpression(t.memberExpression(tcomb, list), [getType(node)]);
  }

  function getMaybe(node) {
    return t.callExpression(t.memberExpression(tcomb, maybe), [getType(node)]);
  }

  function getTuple(nodes) {
    return t.callExpression(t.memberExpression(tcomb, tuple), [t.arrayExpression(nodes.map(getType))]);
  }

  function getUnion(nodes) {
    return t.callExpression(t.memberExpression(tcomb, union), [t.arrayExpression(nodes.map(getType))]);
  }

  function getDict(key, value) {
    return t.callExpression(t.memberExpression(tcomb, dict), [getType(key), getType(value)]);
  }

  function getIntersection(nodes) {
    return t.callExpression(t.memberExpression(tcomb, intersection), [t.arrayExpression(nodes.map(getType))]);
  }
