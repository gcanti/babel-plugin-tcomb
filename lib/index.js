...
exports['default'] = function (_ref) {
  ...
  function getFunctionArgumentChecks(node) {
    ...
    return node.params.filter(getTypeAnnotation).map(function (param) {
      ...
      return t.expressionStatement(t.assignmentExpression('=', id, t.callExpression(getType(typeAnnotation.typeAnnotation), [id])));
    });
  }
  ...
  function getFunctionReturnTypeCheck(node) {
    ...
    return [t.variableDeclaration('const', [t.variableDeclarator(id, t.callExpression(t.memberExpression(t.functionDeclaration(null, params, body), t.identifier('call')), [t.identifier('this')].concat(params)))]), t.returnStatement(t.callExpression(getType(node.returnType.typeAnnotation), [id]))];
  }
