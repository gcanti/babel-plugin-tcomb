...
var flowMagicTypes = {
  ...
  '$Subtype': true,
  '$ObjMap': true,
  '$Values': true // plugin magic types

};
var MAGIC_REFINEMENT_NAME = '$Refinement';
var MAGIC_REIFY_NAME = '$Reify';
var RESERVED_NAMES = (_RESERVED_NAMES = {}, _defineProperty(_RESERVED_NAMES, MAGIC_REFINEMENT_NAME, true), _defineProperty(_RESERVED_NAMES, MAGIC_REIFY_NAME, true), _RESERVED_NAMES); // plugin config
// useful for tests

var SKIP_HELPERS_OPTION = 'skipHelpers'; // useful for keeping the models
...
function _default(_ref) {
  ...
  function getEmptyType() {
    return t.callExpression(t.memberExpression(tcombId, t.identifier('irreducible')), [t.stringLiteral('Empty'), t.functionExpression(null, [], t.blockStatement([t.returnStatement(t.booleanLiteral(false))]))]);
  }
  ...
  function getSpreadCombinator(annotation, typeParameters, name) {
    ...
    var buildItem = function buildItem(node) {
    ...
    var buildProps = function buildProps(props) {
      return t.objectExpression([t.objectProperty(t.stringLiteral('props'), t.objectExpression(props))]);
    };

    astItems.forEach(function (item) {
      if (item.node.type === 'ObjectTypeProperty') {
        var type = item.any ? getAnyType() : getType(item.node.value, typeParameters);
        ...
        params.push(t.objectExpression([t.objectProperty(t.stringLiteral('interface'), getType(item.node)), t.objectProperty(t.stringLiteral('maybe'), t.booleanLiteral(item.maybe))]));
      }
    });
    ...
    return t.callExpression(spreadId, [t.arrayExpression(params), name]);
  } //
  // Flow types
  //
  ...
  function getType(annotation, typeParameters, typeName) {
    switch (annotation.type) {
      ...
      default:
        throw new Error("Unsupported type annotation: ".concat(annotation.type));
    }
  }

  function nodeToString(id) {
    return (0, _generator["default"])(id, {
      concise: true
    }).code;
  }
  ...
  function buildCodeFrameError(path, error) {
    throw path.buildCodeFrameError("[".concat(PLUGIN_NAME, "] ").concat(error.message));
  }
  ...
  function preventReservedNamesUsage(path) {
    ...
    if (name in RESERVED_NAMES) {
      buildCodeFrameError(path, new Error("".concat(name, " is a reserved interface name for ").concat(PLUGIN_NAME)));
    }
  }
  ...
  return {
    visitor: {
      ...
      ExportNamedDeclaration: function ExportNamedDeclaration(path) {
        var node = path.node; // prevent transform-flow-strip-types
      ...
      TypeCastExpression: function TypeCastExpression(path, state) {
        ...
        if (isRuntimeTypeIntrospection(node)) {
          try {
            ...
            buildCodeFrameError(path, new Error("Invalid use of ".concat(MAGIC_REIFY_NAME, ", example: const ReifiedMyType = (({}: any): $Reify<MyType>)")));
          }
        } else {
      ...
      Function: function Function(path, state) {
        ...
        var isAsync = false;
        var typeParameters = assign(getTypeParameters(node), node[TYPE_PARAMETERS_STORE_FIELD]); // store type parameters so we can read them later
