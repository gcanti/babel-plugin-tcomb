...
exports.default = function (_ref) {
  ...
  function getAssert(_ref4, typeParameters) {
    ...
    if (type.type === 'Identifier') {
      type = genericsHelper({ tcomb: tcombId, type: type });
    }
    return t.expressionStatement(t.callExpression(assertId, [id, type, name]));
  }
  ...
  function getInterfaceDefinition(path) {
    ...
    if (node.extends.length === 0) {
      ...
      if (isRecursive) {
        return [defineDeclareCombinator(node), t.callExpression(t.memberExpression(node.id, t.identifier('define')), [t.callExpression(t.memberExpression(t.memberExpression(tcombId, t.identifier(INTERFACE_COMBINATOR_NAME)), t.identifier('extend')), [t.arrayExpression(mixins.map(function (inter) {
          return inter.id;
        }).concat(props))])])];
      }

      return t.variableDeclaration('const', [t.variableDeclarator(node.id, t.callExpression(t.memberExpression(t.memberExpression(tcombId, t.identifier(INTERFACE_COMBINATOR_NAME)), t.identifier('extend')), [t.arrayExpression(mixins.map(function (inter) {
        return inter.id;
  ...
  function isRecursiveType(node) {
    return node.isRecursive || Array.isArray(node.leadingComments) && node.leadingComments.some(function (comment) {
      ...
      ImportDeclaration: function ImportDeclaration(path) {
        ...
        if (node.importKind === 'type') {
          (function () {
            ...
            var nodes = [importNode].concat(node.specifiers.map(function (specifier) {
              return t.variableDeclaration('const', [t.variableDeclarator(specifier.imported, t.logicalExpression('||', t.memberExpression(typesId, specifier.imported), getAnyType()))]);
            }));
